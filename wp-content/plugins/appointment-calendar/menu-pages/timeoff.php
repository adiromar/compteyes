...
<div class="bs-docs-example tooltip-demo" style="background-color: #FFFFFF;">
    <div style="background:#C3D9FF; margin-bottom:10px; padding-left:10px;">
      <h3><?php _e('Time Off','comptoneye','comptoneye'); ?></h3>
    </div>
    <form name="timeoff" id="timeoff" method="post" action="" >
        <table width="100%" border="0" class="table">
            <tr>
            <th scope="col"><?php _e('No.','comptoneye'); ?></th>
            <th scope="col"><?php _e('Name','comptoneye'); ?></th>
            <th scope="col"><?php _e('Date','comptoneye'); ?></th>
            <th scope="col"><?php _e('Time','comptoneye'); ?></th>
            <th scope="col"><?php _e('Repeat','comptoneye'); ?></th>
            <th scope="col"><?php _e('Status','comptoneye'); ?></th>
            <th scope="col"><?php _e('Action','comptoneye'); ?></th>
            <th scope="col" style="text-align: center;"><a rel="tooltip" title="<?php _e('Select All','comptoneye'); ?>"><input type="checkbox" id="checkbox" name="checkbox[]" value="checkbox" /></a></th>
            </tr>
                <?php
                global $wpdb;
            ...
            <tr>
                ...
                <td>
                    ...
                    <?php
                        if($Event->repeat == 'N' && !$Event->allday) {
                            echo _e("None", "comptoneye");
                        }
                        if($Event->repeat == 'PD') {
                            echo _e("Particular Date(s)", "comptoneye");
                        }
                        if($Event->repeat == 'D') {
                            echo _e("Daily", "comptoneye");
                        }
                        if($Event->repeat == 'W') {
                            echo _e("Weekly", "comptoneye");
                        }
                        if($Event->repeat == 'BW') {
                            echo _e("Bi-Weekly", "comptoneye");
                        }
                        if($Event->repeat == 'M') {
                            $diff = ( strtotime($Event->end_date) - strtotime($Event->start_date)  ) /60/60/24/31;
                            echo _e("Monthly", "comptoneye");
                        }
                        if($Event->allday) {
                            echo " ";
                            echo _e("All Day", "comptoneye");
                        }
                    ?>
                    </em>
                </td>
                ...
                <td>
                    <a href="?page=update-time-off&update-timeoff=<?php echo esc_attr($Event->id); ?>" title="<?php _e('Update','comptoneye'); ?>" rel="tooltip"><i class="icon-pencil"></i></a>&nbsp;
                    <a href="?page=timeoff&delete-timeoff=<?php echo esc_attr($Event->id); ?>" title="<?php _e('Delete','comptoneye'); ?>" onclick="return confirm('<?php _e('Do you want to delete this time off?','comptoneye'); ?>')" rel="tooltip"><i class="icon-remove"></i>
                </td>
                <td style="text-align: center;"><a rel="tooltip" title="<?php _e('Select','comptoneye'); ?>"><input type="checkbox" id="checkbox[]" name="checkbox[]" value="<?php echo esc_attr($Event->id); ?>" /></a></td>
            </tr>
                <?php
                    $no++;
                    } // foreach
                ...
                ?>
            <tr>
                <td colspan="2"><button name="addnewtimeoff" id="addnewtimeoff" class="btn btn-info" type="button" ><i class="icon-plus icon-white"></i> <?php _e('Add New Time Off','comptoneye'); ?></button></td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td style="text-align: center;"><button name="deleteall" class="btn btn-danger" type="submit" id="deleteall" onclick="return confirm('<?php _e('Do you want to delete selected Time Off?','comptoneye'); ?>')" ><i class="icon-trash icon-white"></i> <?php _e('Delete','comptoneye'); ?></button></td>
            </tr>
        </table>
    </form>
        ...
        <div class="modal" id="myModal">
            <form action="" method="post" name="AddNewTimeOff-From" id="AddNewTimeOff-From">
                ...
                <div class="modal-info">
                    <div class="alert alert-info"><h4><?php _e('Add New Time Off','comptoneye'); ?></h4></div>
                </div>
                <div class="modal-body">
                    <table width="100%" class="table">
                        <tr>
                            <th scope="row"><?php _e('All Day Time Off','comptoneye'); ?></th>
                            <td><strong>:</strong></td>
                            <td><input name="allday" id="allday" type="checkbox" value="1" /></td>
                        </tr>
                        <tr>
                            <th scope="row"><?php _e('Name','comptoneye'); ?></th>
                            <td><strong>:</strong></td>
                        ...
                        <tr>
                            <th scope="row"><?php _e('Repeat','comptoneye'); ?></th>
                            <td><strong>:</strong></td>
                            <td>
                                <select name="repeat" id="repeat" onchange="repeatday()">
                                    <option onclick="hideAll()" value="N"><?php _e('No','comptoneye'); ?></option>
                                    <option onclick="showPD()" value="PD"><?php _e('Particular Date(s)','comptoneye'); ?></option>
                                    <option onclick="showDaily()" value="D"><?php _e('Daily','comptoneye'); ?></option>
                                    <option onclick="showWeekly()" value="W"><?php _e('Weekly','comptoneye'); ?></option>
                                    <option onclick="showBiWeekly()" value="BW"><?php _e('Bi-Weekly','comptoneye'); ?></option>
                                    <option onclick="showMonthly()" value="M"><?php _e('Monthly','comptoneye'); ?></option>
                                </select>
                            </td>
                        </tr>
                        <tr id="re_days_tr" style="display:none;">
                            <th scope="row"><?php _e('Repeat Day(s)','comptoneye'); ?></th>
                            <td><strong>:</strong></td>
                            <td><input name="re_days" id="re_days" type="text"  maxlength="2"/></td>
                        </tr>
                        <tr id="re_weeks_tr" style="display:none;">
                            <th scope="row"><?php _e('Repeat Week(s)','comptoneye'); ?></th>
                            <td><strong>:</strong></td>
                            <td><input name="re_weeks" id="re_weeks" type="text"  maxlength="2"/></td>
                        </tr>
                        <tr id="re_biweeks_tr" style="display:none;">
                            <th scope="row"><?php _e('Repeat Bi-Week(s)','comptoneye'); ?></th>
                            <td><strong>:</strong></td>
                            <td><input name="re_biweeks" id="re_biweeks" type="text"  maxlength="2"/></td>
                        </tr>
                        <tr id="re_months_tr" style="display:none;">
                            <th scope="row"><?php _e('Repeat Month(s)','comptoneye'); ?></th>
                            <td><strong>:</strong></td>
                            <td><input name="re_months" id="re_months" type="text"  maxlength="2"/></td>
                        </tr>
                        <tr id="start_date_tr" style="display:none;">
                            <th scope="row"><?php _e('Start Date','comptoneye'); ?></th>
                            <td><strong>:</strong></td>
                            <td><input name="start_date" id="start_date" type="text" maxlength="2"/></td>
                        </tr>
                        <tr id="end_date_tr" style="display:none;">
                            <th scope="row"><?php _e('End Date','comptoneye'); ?></th>
                            <td><strong>:</strong></td>
                            <td><input name="end_date" id="end_date" type="text" /></td>
                        </tr>
                        <tr>
                            <th scope="row"><?php _e('Note','comptoneye'); ?></th>
                            <td><strong>:</strong></td>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <button name="create" id="create" class="btn btn-success" type="submit"><i class="icon-ok icon-white"></i> <?php _e('Create Time Off','comptoneye'); ?></button>
                                <a href="#"class="btn btn-danger" id="close"><i class="icon-remove icon-white"></i> <?php _e('Close','comptoneye'); ?></a>
                            </td>
                        </tr>
                    </table>
                </div>
...
<?php 
    if(isset($_POST['create'])) {
        ...
        if($wpdb->query($wpdb->prepare("INSERT INTO `$EventTable` ( `id` , `name` , `allday` , `start_time` , `end_time` , `repeat` ,
            ...
            echo "<script>alert('".__('Time-off successfully added.','comptoneye')."')</script>";
...
<?php 
    if(isset($_GET['delete-timeoff'])) {
        ...
        if($wpdb->query($wpdb->prepare("delete from `$EventTable` where `id` = %s",$del_id))) {
            echo "<script>alert('".__('Time-off successfully deleted.','comptoneye')."')</script>";
            echo "<script>location.href='?page=timeoff';</script>";
        } else {
            echo "<script>location.href='?page=timeoff';</script>";
        }
...
if(isset($_POST['deleteall'])) {
    ...
    if(count($_POST['checkbox'])) {
        echo "<script>alert('".__('Selected Time-off successfully deleted.','comptoneye')."')</script>";
    } else {
        echo "<script>alert('".__('No time-off selected to delete.','comptoneye')."')</script>";
    }
    echo "<script>location.href='?page=timeoff';</script>";
}
?>
...
jQuery(document).ready(function () {
    ...
    jQuery('#create').click(function() {
        ...
        if(nameVal == '') {
            jQuery("#name").after('<span class="error"><br><strong><?php _e('This field required.','comptoneye'); ?></strong></span>');
            return false;
        }
        ...
        if (!jQuery('#allday').is(':checked')) {
            ...
            if(start_time == '') {
                jQuery("#start_time").after('<span class="error"><br><strong><?php _e('This field required.','comptoneye'); ?></strong></span>');
                return false;
            }
            ...
            if(end_time == ''){
                jQuery("#end_time").after('<span class="error"><br><strong><?php _e('This field required.','comptoneye'); ?></strong></span>');
                return false;
            }
            ...
            if(stt > endt) {
                ...
                jQuery("#end_time").after('<span class="error"><br><strong><?php _e("End-time must be bigger then Start-time.",'comptoneye'); ?></strong></span>');
                return false;
            }
        }
        ...
        if(repeat == 'PD'){
            ...
            if(start_date == '') {
                jQuery("#start_date").after('<span class="error"><br><strong><?php _e('This field required.','comptoneye'); ?></strong></span>');
                return false;
            }
            var end_date = jQuery("#end_date").val();
            if(end_date == '') {
                jQuery("#end_date").after('<span class="error"><br><strong><?php _e('This field required.','comptoneye'); ?></strong></span>');
                return false;
            }
        }
        ...
        if(repeat == 'D'){
            ...
            if(re_days == '')
            {
                jQuery("#re_days").after('<span class="error"><br><strong><?php _e('This field required.','comptoneye'); ?></strong></span>');
                return false;
            }
        }
        ...
        if(repeat == 'W'){
            ...
            if(re_weeks == '')
            {
                jQuery("#re_weeks").after('<span class="error"><br><strong><?php _e('This field required.','comptoneye'); ?></strong></span>');
                return false;
            }
        }
        ...
        if(repeat == 'BW'){
            ...
            if(re_biweeks == '')
            {
                jQuery("#re_biweeks").after('<span class="error"><br><strong><?php _e('This field required.','comptoneye'); ?></strong></span>');
                return false;
            }
        }
        ...
        if(repeat == 'M'){
            ...
            if(re_months == '')
            {
                jQuery("#re_months").after('<span class="error"><br><strong><?php _e('This field required.','comptoneye'); ?></strong></span>');
                return false;
            }
        }
        ...
        if(repeat != 'PD'){
            ...
            if(event_date == ''){
                jQuery("#event_date").after('<span class="error"><br><strong><?php _e('This field required.','comptoneye'); ?></strong></span>');
                return false;
            }
        }
    });
