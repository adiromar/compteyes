...
<div style="background:#C3D9FF; margin-bottom:10px; padding-left:10px;">
  <h3><?php _e('Update Time Off','comptoneye'); ?></h3> 
</div>
    ...
    if(isset($_GET['update-timeoff'])) {
        ...
        <form action="" method="post" name="AddNewTimeOff-From" id="AddNewTimeOff-From">
            ...
            <table width="100%" class="table">
                <tr>
                    <th width="21%" scope="row"><?php _e('All Day Time Off','comptoneye'); ?></th>
                ...
                <tr>
                    ...
                    <td>
                        <select name="repeat" id="repeat" onchange="repeatday()">
                            <option onclick="hideAll()" value="N" <?php if($TimeOff->repeat == 'N') echo "selected=selected"; ?> ><?php _e('No','comptoneye'); ?></option>
                            <option onclick="showPD()" value="PD" <?php if($TimeOff->repeat == 'PD') echo "selected=selected"; ?> ><?php _e('Particular Date(s)','comptoneye'); ?></option>
                            <option onclick="showDaily()" value="D" <?php if($TimeOff->repeat == 'D') echo "selected=selected"; ?> ><?php _e('Daily','comptoneye'); ?></option>
                            <option onclick="showWeekly()" value="W" <?php if($TimeOff->repeat == 'W') echo "selected=selected"; ?> ><?php _e('Weekly','comptoneye'); ?></option>
                            <option onclick="showBiWeekly()" value="BW" <?php if($TimeOff->repeat == 'BW') echo "selected=selected"; ?> ><?php _e('Bi-Weekly','comptoneye'); ?></option>
                            <option onclick="showMonthly()" value="M" <?php if($TimeOff->repeat == 'M') echo "selected=selected"; ?> ><?php _e('Monthly','comptoneye'); ?></option>
                        </select>
                    </td>
                </tr>
                ...
                <tr id="re_weeks_tr" style="display:none;">
                    <th scope="row"><?php _e('Repeat Weeks(s)','comptoneye'); ?></th>
                    <td><strong>:</strong></td>
                ...
                <tr>
                    ...
                    <td>&nbsp;</td>
                    <td>
                        <button name="create" id="create" class="btn btn-success" type="submit"><i class="icon-pencil icon-white"></i> <?php _e('Update','comptoneye'); ?></button>
                        <?php if(isset($_GET['from'])) { ?>
                        <a href="?page=appointment-calendar" class="btn btn-danger"><i class="icon-remove icon-white"></i> <?php _e("Cancel", "comptoneye"); ?></a></td>
                    <?php } else { ?>
                        <a href="?page=timeoff" class="btn btn-danger"><i class="icon-remove icon-white"></i> <?php _e("Cancel", "comptoneye"); ?></a></td>
                    <?php } ?>
                    </td>
                </tr>
            </table>
...
<?php 
    if(isset($_POST['create'])) {
        ...
        if($FromBack) {
            echo "<script>alert('".__('Time-off successfully updated.','comptoneye')."')</script>";
...
<?php 
    if(isset($_GET['delete-timeoff'])) {
        ...
        if($wpdb->query($wpdb->prepare("delete from `$EventTable` where `id` = %s",$DelId))) {
            echo "<script>alert('".__('Time Off deleted successfully.','comptoneye')."')</script>";
            echo "<script>location.href='?page=timeoff';</script>";
        } else {
            echo "<script>location.href='?page=timeoff';</script>";
        }
