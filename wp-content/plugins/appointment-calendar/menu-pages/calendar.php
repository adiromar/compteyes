...
jQuery(document).ready(function() {
        ...
        events: [
            ...
            if($AllAppointments) {
                foreach($AllAppointments as $single) {
                    ...
                    {
                        title: "<?php _e('Booked By', 'comptoneye'); echo ': '; echo $title; ?>",
    ...
    jQuery('#next1').click(function(){
        ...
        if(jQuery('#service').val() == 0) {
            jQuery("#service").after("<span class='error'><p><strong><?php echo __('Select any service.', 'comptoneye'); ?></strong></p></span>");
            return false;
        }
...
function checkvalidation() {
    ...
    if(!start_time) {
        jQuery("#clientboxform").before("<span class='error'><p align=center><strong><?php echo __('Select any time.', 'comptoneye'); ?></strong></p></span>");
        return false;
    }

    var name = jQuery("#name").val();
    if(name == '') {
        jQuery("#name").after("<span class='error'><p><strong><?php echo __('Name required.', 'comptoneye'); ?></strong></p></span>");
        return false;
    } else {
        var name = isNaN(name);
        if(name == false) {
            jQuery("#name").after("<span class='error'><p><strong><?php echo __('Invalid name.', 'comptoneye'); ?></strong></p></span>");
            return false;
        }
    }
    ...
    if(email == '') {
        jQuery("#email").after("<span class='error'><p><strong><?php echo __('Email required.', 'comptoneye'); ?></strong></p></span>");
        return false;
    } else {
        if(regex.test(email) == false ) {
            jQuery("#email").after("<span class='error'><p><strong><?php echo __('Invalid email.', 'comptoneye'); ?></strong></p></span>");
            return false;
        }
    }
    ...
    if(phone == '') {
        jQuery("#phone").after("<span class='error'><p><strong><?php echo __('Phone required.', 'comptoneye'); ?></strong></p></span>");
        return false;
    } else {
        var phone = isNaN(phone);
        if(phone == true) {
            jQuery("#phone").after("<span class='error'><p><strong><?php echo __('Invalid phone number.', 'comptoneye'); ?></strong></p></span>");
            return false;
        }
    }
    ...
    jQuery.ajax({
        ...
        success: function(data) {
            ...
            alert("<?php echo _e("Thank you, appointment has been scheduled. Confirmation mail sent to the client and admin.", "comptoneye"); ?>");
            ...
            <form action="" method="post" name="addnewappointment" id="addnewappointment">
                <div class="modal-info">
                    ...
                    <div class="alert alert-info">
                        <h4><?php _e("Schedule New Appointment", "comptoneye"); ?></h4><?php _e("Select Date & Service", "comptoneye"); ?>
                    </div>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div id="secdiv" style="float:right;" >
                        <p><strong><?php echo _e("Your Appointment Date", "comptoneye"); ?>:</strong></p>
                        ...
                        <p><strong><?php _e("Select Service", "comptoneye"); ?>:</strong></p>
                        <select name="service" id="service">
                            <option value="0"><?php _e("Select any service", "comptoneye"); ?></option>
                        ...
                        <br>
                        <button name="next1" class="btn" type="button" id="next1"><?php echo _e("Next", "comptoneye"); ?> <i class="icon-arrow-right"></i> </button>
